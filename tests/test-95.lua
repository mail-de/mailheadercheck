...
mt.echo("*** begin test-95 - Missing Message-ID, From address NOT in exclude_fromheader_addresses (REJECT)")

-- start the filter
mt.startfilter("./mailheadercheck", "--config", "tests/test-95-config.yaml")
...
mt.macro(conn, SMFIC_CONNECT, "i", "test-id-95")
...
-- send headers
if mt.header(conn, "From", "\"Other\" <other@example.org>") ~= nil then

...
mt.echo("*** begin test-52 - 1 From, 1 Subject, 1 Date, 2 Sender")
...
if mt.header(conn, "Sender", "Sender1 display name <sender1@example.com>") ~= nil then
     error "mt.header(Sender) failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
     error "mt.header(Sender) unexpected reply"
end
if mt.header(conn, "Sender", "Sender2 display name <sender2@example.com>") ~= nil then
     error "mt.header(Sender) failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
     error "mt.header(Sender) unexpected reply"

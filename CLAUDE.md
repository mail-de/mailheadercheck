...
1. **Functional milter tests** — runs each `tests/test-*.lua` file via `miltertest`.
...
**Config validation** (`Cfg.validate_config`) is invoked when `--configcheck` is passed and also on every normal startup. It returns `(errors, warnings)` — errors block startup/reload, warnings do not. It validates `log_target`, `log_format`, `debug`, `log_privacy_mode`, `add_result_header`, global `dry_run`, `syslog_facility`, `socket` format, `metrics` section keys and values, and all per-check option names and values. Unknown top-level config keys produce a warning (printed/logged) but do not block startup. `check_dependencies()` from `metrics.py` is also called in both paths to verify `prometheus_client` is installed when `metrics.enabled: 1`.

...
mt.echo("*** begin test-01 - 0 From, 0 Subject, 0 Date")
...
-- send headers
if mt.header(conn, "X-SomeHeader", "testmail abc") ~= nil then
     error "mt.header(X-SomeHeader) failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
     error "mt.header(X-SomeHeader) unexpected reply"
end

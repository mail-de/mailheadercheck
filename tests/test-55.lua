...
mt.echo("*** begin test-55 - 1 From, 1 Subject, 1 Date, 2 Reply-To")
...
if mt.header(conn, "Reply-To", "Reply-To1 display name <replyto1@example.com>") ~= nil then
     error "mt.header(Reply-To) failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
     error "mt.header(Reply-To) unexpected reply"
end
if mt.header(conn, "Reply-To", "Reply-To2 display name <replyto2@example.com>") ~= nil then
     error "mt.header(Reply-To) failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
     error "mt.header(Reply-To) unexpected reply"
